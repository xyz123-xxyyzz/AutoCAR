...
document.addEventListener('DOMContentLoaded', () => {
  ...
  btnAnalyze.addEventListener('click', () => {
    ...
    // Zaman hesab─▒ (Her 100 ilan 1 par├ğa, par├ğa ba┼ş─▒ 2 dakika)
    const chunks = Math.ceil(currentAdsCount / 100);
    const estimatedTime = chunks * 2;
    
    // Maliyet hesab─▒ (1 ─░lan = 0.033 TL)
    const costTL = (currentAdsCount * 0.033).toFixed(2);
    
    modalAdsCount.textContent = `Toplam Se├ğilen ─░lan: ${currentAdsCount}`;
    modalTime.textContent = `Tahmini S├╝re: ~${estimatedTime} dakika (${chunks} i┼şlem par├ğas─▒)`;
  ...
  function checkState() {
    chrome.runtime.sendMessage({ action: 'get_state' }, (response) => {
      ...
      if (response.collectedCount !== undefined) {
        ...
        collectedCounterText.textContent = `Toplanan ─░lan: ${currentAdsCount}`;
  ...
  function renderTabs(tabs) {
    ...
    if (!tabs || tabs.length === 0) {
      tabList.innerHTML = '<div class="empty-state">─░lanlar─▒ farenin orta tu┼şuyla yan sekmede a├ğ─▒n. Otomatik toplanacakt─▒r.</div>';
      return;
    }
