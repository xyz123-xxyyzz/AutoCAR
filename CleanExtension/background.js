...
async function runFullAnalysis() {
  ...
  try {
    ...
    // -------------------------------------------------------------
    // SISTEM KONTROL AŞAMALARI (2 TUR HATA ONARIMI / RETRY AŞAMASI)
    // -------------------------------------------------------------
    for (let round = 1; round <= 2; round++) {
      ...
      if (failedItems.length === 0) {
        break; // Hata kalmadıysa döngüden erken çık
      }

      updateState({ 
        aiStatusText: `Sistem Kontrolü (Aşama ${round}/2): Hata alan ${failedItems.length} araç tespit edildi. Yeniden analiz ediliyor...`,
        analysisProgress: 75 + (round * 5)
      });
      ...
      for (let j = 0; j < failedItems.length; j += RETRY_CHUNK_SIZE) {
        ...
        const retryPromises = retryChunk.map(async (failedItem) => {
          ...
          retryProcessed++;
          updateState({
            aiStatusText: `Sistem Kontrolü (Aşama ${round}/2) - Yeniden Deneniyor (${retryProcessed}/${failedItems.length})...`
          });
        ...
        if (j + RETRY_CHUNK_SIZE < failedItems.length && !isAnalysisCancelled) {
          updateState({ 
            aiStatusText: `Sistem Kontrolü (Aşama ${round}/2) - Limit koruması: 3 saniye bekleniyor...`,
          });
          await new Promise(r => setTimeout(r, 3000));
        }
      }
